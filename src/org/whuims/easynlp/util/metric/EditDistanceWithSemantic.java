...
import org.whuims.easynlp.dict.CiLin;
 ...
 * @since 1.0
 */
public class EditDistanceWithSemantic extends EditDistance implements
        Similarity<String> {
    ...
    public EditDistanceWithSemantic() {
        wordlen = 9;
        CiLin dict = new CiLin("resource/dict/synonym/同义词词林扩展版.txt");
        synSet = dict.getSynSet();
    }
                ...
                for (int ii = 1; ii <= wordlen; ii++) {
                    ...
                    for (int jj = 1; jj <= wordlen; jj++) {
                        if (jj + j - 1 > str2.length())
                            break;
                        String combine = str1.substring(i - 1, ii + i - 1)
                                + " | " + str2.substring(j - 1, jj + j - 1);
                         System.out.println(combine);
                        if (synSet.contains(combine)) {
                            if (d[i + ii - 1][j + jj - 1] > 0)
                                d[i + ii - 1][j + jj - 1] = Math.min(d[i + ii
                                        - 1][j + jj - 1],
                                        d[i - 1][j - 1] + 0.1f);
                            else
