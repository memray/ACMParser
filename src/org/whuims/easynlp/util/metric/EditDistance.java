...
public class EditDistance {
	public float calcNormalise(String word, String word2) {
		...
		int len = word.length() > word2.length() ? word.length() : word2
				.length();
	...
	public float calc(String cSeq1, String cSeq2) {
		...
		for (int y = 1; y < ysLength; ++y)
			currentSlice[y] = currentSlice[y - 1]
					+ costIns(cSeq2.charAt(y - 1)); // y inserts down first
													// column of lattice
		...
		for (int x = 1; x < xsLength; ++x) {
			...
			// ///////////////////////////
			currentSlice[0] = lastSlice[0] + costDel(cSeq1.charAt(x - 1)); // x
																			// deletes
																			// across
																			// first
																			// row
			...
			for (int y = 1; y < ysLength; ++y) {
				...
				// match
				currentSlice[y] = Math.min(cX == cY ? lastSlice[yMinus1] // match
	...
	public static void main(String[] args) {
		...
		System.out.println(ed.calc(a, b));
	}
}
